...
const onSubmitOTP = async (e) => {
  ...
  try {
    const { data } = await axios.post(backendUrl + '/api/auth/verify-reset-otp', { otp: enteredOtp });

    if (data.success) {
      toast.success(data.message);
      setOtp(enteredOtp);
      setIsOtpSubmitted(true); // now show the "Enter New Password" form
      // Optional: if backend sends email, setEmail(data.email);
    } else {
      toast.error(data.message);
    }
  } catch (error) {
...
const onSubmitNewPassword = async (e) => {
  ...
  <img onClick={() => navigate('/')} src={assets.logo} alt="" className='absolute left-5 sm:left-20 top-5 w-10 sm:w-16 cursor-pointer' />
